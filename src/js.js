...
export class DatePicker extends Component {
    ...
    returnDefaultSelectTime = (props) => {
        if (this.returnPickerType(props) !== 'time') {
            const selectTime = tprops.defaultSelectTime || this.pickerTimeInterval[0]
            ...
            Utils.Toast(new Date(defaultSelectTime).getHours())
            return defaultSelectTime.toString
        }
    }
    replaceDate = (date) => {
    ...
    returnYears = (props) => {
        const startYear = Number(this.returnStartDateTime(props).split('-')[0])
        const endYear = Number(this.returnEndDateTime(props).split('-')[0])
        let yearsList = []
